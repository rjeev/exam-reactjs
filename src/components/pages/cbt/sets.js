...
const CbtSets = () => {
  ...
  const getCbtSets = async () => {
    setLoading(true);
    const category_id = userData.user.cbt_category_id;
    const response = await fetch(`${process.env.REACT_APP_apiUrl}api/v1/cbt/set/${category_id}/all`, {
      method: "GET",
      headers: {
  ...
  const addToYourList = async (set_id) => {
    const response = await fetch(`${process.env.REACT_APP_apiUrl}api/v1/cbt/set/${set_id}`, {
        method: "POST",
        headers: {
          'Access-Control-Allow-Origin': '*',
          'Authorization': 'Bearer '+ userData.token,
  ...
  const startExam = async (set_id) => {
    navigate(`/cbt/start/${set_id}`);

    const response = await fetch(`${process.env.REACT_APP_apiUrl}api/v1/cbt/set/${set_id}/reset`, {
      method: "POST",
      headers: {
        'Authorization': 'Bearer '+ userData.token,
        'Content-Type': 'application/json'
    ...
    <div id="wrapper">
     <div className="container-fluid">
      ...
      <form action={`${process.env.REACT_APP_esewa_url}`} ref={formRef} method="POST" name="test">
        ...
        <input value={`${process.env.REACT_APP_domain}/purchase?status=pass&type=cbt`} type="hidden" name="su"/>
        <input value={`${process.env.REACT_APP_domain}/purchase?status=fail&type=cbt`} type="hidden" name="fu"/>
      </form>
    </div>
  );
};
