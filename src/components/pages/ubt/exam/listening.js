...
const UbtListening = () => {
  ...
  const getQuestion = async (sid) => {
    ...
    const resp = await fetch(
      `${process.env.REACT_APP_apiUrl}api/v1/ubt/set/${sid}/question/${question_no}/listening`,
      {
        method: "GET",
        headers: {
    ...
    if (resp.ok) {
      if (data.status) {
        if (data.payload.ubt) {
          setQuestion(data.payload.ubt[0]);
        } else {
  ...
  const checkSetExist = () => {
    if (
      ...
      setSetId(localStorage.getItem("ubt_set_id"));
        ...
        if (data.payload) {
          if (data.payload.answers.length > 0) {
            ...
            data.payload.answers.map((ans, key) => {
              arr1[ans.ubt_question_id] = ans.answer;
            });
  ...
  const getPreviousQuestion = () => {
    ...
    if (currentQuestion > 0) {
      ...
      navigate(`/ubt/exam/${type}?question_no=${currentQuestion}`);
    } else {
...
UbtListening.layout = "L2";
export default UbtListening;
