...
const UbtDashboard = () => {
  ...
  const getSetDetail = async (sid) => {
    setLoading(true);
    const resp = await fetch(`${process.env.REACT_APP_apiUrl}api/v1/ubt/${sid}`, {
      method: "GET",
      headers: {
  ...
  const getAnswers = async (sid) => {
    const resp = await fetch(
      `${process.env.REACT_APP_apiUrl}api/v1/ubt/set/${sid}/answers`,
      {
        method: "GET",
        headers: {
          Authorization: "Bearer " + userData.token,
  ...
  const chooseAnswer = async (id, qno, option, type) => {
    ...
    const resp = await fetch(`${process.env.REACT_APP_apiUrl}api/v1/ubt/answer`, {
      method: "POST",
  ...
  const getQuestions = async (sid) => {
    ...
    const resp = await fetch(
      `${process.env.REACT_APP_apiUrl}api/v1/ubt/${sid}/questions`,
      {
        method: "GET",
        headers: {
          Authorization: "Bearer " + userData.token,
  ...
  const checkSetExist = () => {
    if (
      localStorage.getItem("ubt") !== undefined &&
      localStorage.getItem("ubt") * 1 > 0
    ) {
      setSetId(localStorage.getItem("ubt"));
      return localStorage.getItem("ubt");
    } else {
      return false;
    }
  };
  ...
  const gotoSingleQuestion = (question) => {
    ...
    navigate(`/ubt/exam/${type}?question_no=${question}`);
  };
...
UbtDashboard.layout = "L2";
export default UbtDashboard;
